...
static int addr_in_range(void __iomem *virt_base, unsigned int size, void *addr)
{
 ...
 */
static struct smem_partition_header __iomem *
qcom_smem_partition_header(struct qcom_smem *smem,
	struct smem_ptable_entry *entry, u16 host0, u16 host1, u32 phys_addr)
{
...
static int qcom_smem_set_global_partition(struct qcom_smem *smem)
{
	...
	header = qcom_smem_partition_header
		(smem, entry, SMEM_GLOBAL_HOST, SMEM_GLOBAL_HOST, phys_addr);
	if (!header)
		return -EINVAL;
...
qcom_smem_enumerate_partitions(struct qcom_smem *smem, u16 local_host)
{
	...
	for (i = 0; i < le32_to_cpu(ptable->num_entries); i++) {
		...
		phys_addr = smem->regions[0].aux_base + le32_to_cpu(entry->offset);
		header = qcom_smem_partition_header(smem, entry, host0, host1, phys_addr);
		if (!header)
			return -EINVAL;
