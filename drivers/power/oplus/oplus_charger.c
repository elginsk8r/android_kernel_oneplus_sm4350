...
static struct drm_panel *lcd_active_panel = NULL;
...
static void oplus_chg_check_tled_skin_status(struct oplus_chg_chip *chip);
#ifdef  CONFIG_FB
...
static void oplus_chg_load_lcd_panel(struct work_struct *work);
		...
		const union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_SHIP_MODE:
			...
			if(!chip->chg_ops->set_shipmode)
			{
				pr_err("set_shipmode failed %d \n", psp);
				chip->enable_shipmode = 0;
				ret = -EINVAL;
				break;
			}
			ret = chip->chg_ops->set_shipmode(chip->enable_shipmode);
			pr_err("set POWER_SUPPLY_PROP_SHIP_MODE done, ret = %d!\n", ret);
			break;
#endif
#ifdef CONFIG_OPLUS_CALL_MODE_SUPPORT
		case POWER_SUPPLY_PROP_CALL_MODE:
					...
					size_t count, loff_t *lo)
{
	...
	if(buffer[0] == '5'){
		...
		chg_err("charger_factorymode_test get 2,do nothing\n");
	} else if(buffer[0] == '1'){
...
int oplus_chg_init(struct oplus_chg_chip *chip)
{
	...
	schedule_work(&chip->load_lcd_work);
	pr_err("schedule_work DONE!!!!!!\n");
...
EXPORT_SYMBOL(oplus_chg_parse_swarp_dt);
...
static int check_dt(struct device_node *np)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (node == NULL) {
			pr_err("[Charge]: %s in %d NULL point\n", __func__, __LINE__);
			return -ENODEV;
		}
		chg_err("node name is %s\n", node->name);
		retry_count = 0;
		while(panel_status && retry_count < 3) {
			panel = of_drm_find_panel(node);
			if (!IS_ERR(panel)) {
				pr_err("charge matched lcd panel\n");
				panel_status = false;
...
static void oplus_chg_load_lcd_panel(struct work_struct *work)
{
	...
	while (rc != 0 && retry_count--) {
		chg_err("check_dt fail, try again\n");
		usleep_range(3000, 3000);
		rc = check_dt(node);

		if (rc == 0) {
			isOK = 1;
			pr_debug("get check_dt ok\n");
			break;
		}
	}
	if (!rc || isOK) {
		...
		if (lcd_active_panel) {
			...
			pr_err("[OPLUS_CHG][%s]: lcd_active_panel drm_panel_notifier_register ERROR!!!\n", __func__);
			rc = -1;
		}
#else
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "op,nor_skin_thermal_normal_threshold",
			&chip->limits.nor_skin_thermal_normal_threshold);
	if (rc < 0) {
		chip->limits.nor_skin_thermal_normal_threshold = 37000;
	}
	chg_err("nor_skin_thermal_normal_threshold:%d\n", chip->limits.nor_skin_thermal_normal_threshold);

	rc = of_property_read_u32(node, "op,nor_skin_thermal_pre_warm_threshold",
			&chip->limits.nor_skin_thermal_pre_warm_threshold);
	if (rc < 0) {
		chip->limits.nor_skin_thermal_pre_warm_threshold = 39000;
	}
	chg_err("nor_skin_thermal_pre_warm_threshold:%d\n", chip->limits.nor_skin_thermal_pre_warm_threshold);

	rc = of_property_read_u32(node, "op,nor_skin_thermal_warm_threshold",
			&chip->limits.nor_skin_thermal_warm_threshold);
	if (rc < 0) {
		chip->limits.nor_skin_thermal_warm_threshold = 41000;
	}
	chg_err("nor_skin_thermal_warm_threshold:%d\n", chip->limits.nor_skin_thermal_warm_threshold);

	rc = of_property_read_u32(node, "op,nor_skin_thermal_pre_high_threshold",
			&chip->limits.nor_skin_thermal_pre_high_threshold);
	if (rc < 0) {
		chip->limits.nor_skin_thermal_pre_high_threshold = 43000;
	}
	chg_err("nor_skin_thermal_pre_high_threshold:%d\n", chip->limits.nor_skin_thermal_pre_high_threshold);

	rc = of_property_read_u32(node, "op,nor_skin_thermal_high_threshold",
			&chip->limits.nor_skin_thermal_high_threshold);
	if (rc < 0) {
		chip->limits.nor_skin_thermal_high_threshold = 45000;
	}
	chg_err("nor_skin_thermal_high_threshold:%d\n", chip->limits.nor_skin_thermal_high_threshold);

	rc = of_property_read_u32(node, "op,input_current_led_ma_skin_normal",
			&chip->limits.input_current_led_ma_skin_normal);
	if (rc < 0) {
		chip->limits.input_current_led_ma_skin_normal = 2000;
	}

	rc = of_property_read_u32(node, "op,input_current_led_ma_skin_warm",
			&chip->limits.input_current_led_ma_skin_warm);
	if (rc < 0) {
		chip->limits.input_current_led_ma_skin_warm = 1500;
	}

	rc = of_property_read_u32(node, "op,input_current_led_ma_skin_med",
			&chip->limits.input_current_led_ma_skin_med);
	if (rc < 0) {
		chip->limits.input_current_led_ma_skin_med = 1200;
	}

	rc = of_property_read_u32(node, "op,input_current_led_ma_skin_high",
			&chip->limits.input_current_led_ma_skin_high);
	if (rc < 0) {
		chip->limits.input_current_led_ma_skin_high = 900;
	}

	rc = of_property_read_u32(node, "qcom,normal_current_cdp_ma",
			&chip->limits.normal_current_cdp_ma);
	if (rc) {
		chip->limits.normal_current_cdp_ma = 1500;
	}

	INIT_WORK(&chip->load_lcd_work, oplus_chg_load_lcd_panel);
	charger_xlog_printk(CHG_LOG_CRTI,
	...
	chip->limits.input_current_limit = 2000;
	chip->limits.charge_current_limit = chip->limits.temp_normal_fastchg_current_ma;
...
static void oplus_chg_set_charging_current(struct oplus_chg_chip *chip)
{
	...
	if ((chip->charger_type == POWER_SUPPLY_TYPE_USB_CDP) &&
				(charging_current > chip->limits.normal_current_cdp_ma)) {
		charging_current = chip->limits.normal_current_cdp_ma;
	}
	if (chip->chg_ops->get_charger_subtype() != CHARGER_SUBTYPE_QC || chip->set_5V2A) {
		if (charging_current > 2000) {
			charging_current = 2000;
		}
		charger_xlog_printk(CHG_LOG_CRTI,
				"[BATTERY]5V2A charge, charging current: %d\n", charging_current);
	}
	if (charging_current == 0) {
		return;
	}
	chip->limits.charge_current_limit = charging_current;
...
void oplus_chg_set_input_current_limit(struct oplus_chg_chip *chip)
{
	...
	chip->skin_temp = chip->chg_ops->get_skin_temp();
	if ((chip->chg_ctrl_by_lcd) && (chip->led_on)) {
		pr_err("[BATTERY]LED ON,BEFORE current_limit[%d]\n", current_limit);
		...
		if (chip->dual_charger_support) {	// for skin_temp
			if (chip->skin_temp <= chip->limits.nor_skin_thermal_normal_threshold ||							// <= 37C  2A
					(chip->skin_temp < chip->limits.nor_skin_thermal_pre_warm_threshold_antishake && chip->isSkinTempUp)) {	//[37,39) temp up from <37C use 2A
				if (current_limit > chip->limits.input_current_led_ma_skin_normal){
					current_limit = chip->limits.input_current_led_ma_skin_normal;
				}
			} else if (chip->led_skin_temp_status == LED_SKIN_TEMP_STATUS__LITTLE_WARM  ||						//39C ~ 41C   1.5A
					(chip->led_skin_temp_status == LED_SKIN_TEMP_STATUS__NORMAL && chip->skin_temp > chip->limits.nor_skin_thermal_normal_threshold && !chip->isSkinTempUp) ||	//37C ~ 39C  (37,39] 1.5A
					(chip->led_skin_temp_status == LED_SKIN_TEMP_STATUS__WARM && chip->isSkinTempUp)) {			//[41,43) temp up from 39C~41C use 41C~43C  1.5A
				if (current_limit > chip->limits.input_current_led_ma_skin_warm){
					current_limit = chip->limits.input_current_led_ma_skin_warm;
				}
			} else if ((chip->led_skin_temp_status == LED_SKIN_TEMP_STATUS__WARM && !chip->isSkinTempUp) ||		//41C ~ 43C  temp down from  43C ~ 45C  use 43C ~ 45C  1.2A
					(chip->led_skin_temp_status == LED_SKIN_TEMP_STATUS__LITTLE_HIGH && chip->isSkinTempUp)) {	//43C ~ 45C  or [43, 45) temp up from 41C~43C  use 43C~45C 1.2A
				if (current_limit > chip->limits.input_current_led_ma_skin_med) {
					current_limit = chip->limits.input_current_led_ma_skin_med;
				}
			} else if (chip->led_skin_temp_status == LED_SKIN_TEMP_STATUS__HIGH || chip->led_skin_temp_status == LED_SKIN_TEMP_STATUS__LITTLE_HIGH) {		// >= 45C  0.9A   or  [43, 45) temp down from  > 45C  use >45C   0.9A
				if (current_limit > chip->limits.input_current_led_ma_skin_high){
					current_limit = chip->limits.input_current_led_ma_skin_high;
				}
			} else {
				pr_err("[BATTERY]LED ON, NOT FOUND the right limit!\n");
				current_limit = chip->limits.input_current_led_ma_skin_normal;
			}
			pr_err("[BATTERY]LED STATUS CHANGED, IS ON, SKIN_THERM  isSkinTempUp[%d] , current_limit[%d]\n", chip->isSkinTempUp, current_limit);
		}
	...
	charger_xlog_printk(CHG_LOG_CRTI,
		...
		current_limit = %d, \
		led_temp_status = %d, \
		led_skin_temp_status = %d\n",
		chip->led_on,
		current_limit,
		chip->led_temp_status,
		chip->led_skin_temp_status);
	chip->limits.input_current_limit = current_limit;
...
static bool oplus_chg_check_time_is_good(struct oplus_chg_chip *chip)
{
	...
	if (get_eng_version() != AGING_TEST_STATUS_DEFAULT) {
		chip->chging_over_time = false;
		printk("Not AGING_TEST_STATUS_DEFAULT, do not stop charging by overtime!\n");
		return true;
	}
		...
		unsigned long event, void *data)
{
	...
	if (event == DRM_PANEL_EARLY_EVENT_BLANK) {
		...
		if (blank == DRM_PANEL_BLANK_UNBLANK_CUST) {
			g_charger_chip->led_on = true;
			g_charger_chip->led_on_change = true;
		} else if (blank == DRM_PANEL_BLANK_POWERDOWN_CUST) {
			g_charger_chip->led_on = false;
			g_charger_chip->led_on_change = true;
		} else {
			pr_err("%s: receives wrong data EARLY_BLANK:%d\n", __func__, blank);
		}
		chg_debug("[OPLUS_CHG][%s]: g_charger_chip->led_on=%d，g_charger_chip->led_on_change = %d\n", __func__, g_charger_chip->led_on, g_charger_chip->led_on_change);
	}
	return 0;
}
#else
...
#define TLED_SKIN_HYSTERISIS_DECIDEGC 800
static void oplus_chg_check_tled_skin_status(struct oplus_chg_chip *chip)
{
	OPLUS_CHG_SKIN_TLED_STATUS tled_skin_status = chip->led_skin_temp_status;
	static int skin_high_counts = 0, skin_prehigh_counts = 0, skin_warm_counts = 0, skin_prewarm_counts = 0, normal_counts = 0;
	chip->skin_temp = chip->chg_ops->get_skin_temp();

	if (chip->skin_temp > chip->limits.nor_skin_thermal_high_threshold_antishake) {		/* >45C */
		skin_high_counts ++;
		if (skin_high_counts >= TLED_CHANGE_COUNTS) {
			skin_high_counts = 0;
			skin_prehigh_counts = 0;
			skin_warm_counts = 0;
			skin_prewarm_counts = 0;
			normal_counts = 0;
			tled_skin_status = LED_SKIN_TEMP_STATUS__HIGH;
		}
	} else if (chip->skin_temp > chip->limits.nor_skin_thermal_pre_high_threshold_antishake) {	/* >43C && <= 45 */
		skin_prehigh_counts ++;
		if (skin_prehigh_counts >= TLED_CHANGE_COUNTS) {
			skin_high_counts = 0;
			skin_prehigh_counts = 0;
			skin_warm_counts = 0;
			skin_prewarm_counts = 0;
			normal_counts = 0;
			tled_skin_status = LED_SKIN_TEMP_STATUS__LITTLE_HIGH;
		}
	} else if (chip->skin_temp > chip->limits.nor_skin_thermal_warm_threshold_antishake) {	/* >41C && <= 43 */
		skin_warm_counts ++;
		if (skin_warm_counts >= TLED_CHANGE_COUNTS) {
			skin_high_counts = 0;
			skin_prehigh_counts = 0;
			skin_warm_counts = 0;
			skin_prewarm_counts = 0;
			normal_counts = 0;
			tled_skin_status = LED_SKIN_TEMP_STATUS__WARM;
		}
	} else if (chip->skin_temp > chip->limits.nor_skin_thermal_pre_warm_threshold_antishake) {	/* >39C && <= 41 */
		skin_prewarm_counts ++;
		if (skin_prewarm_counts >= TLED_CHANGE_COUNTS) {
			skin_high_counts = 0;
			skin_prehigh_counts = 0;
			skin_warm_counts = 0;
			skin_prewarm_counts = 0;
			normal_counts = 0;
			tled_skin_status = LED_SKIN_TEMP_STATUS__LITTLE_WARM;
		}
	} else {	/*  <= 39 */
		normal_counts ++;
		if (normal_counts >= TLED_CHANGE_COUNTS) {
			...
			skin_prewarm_counts = 0;
			normal_counts = 0;
			tled_skin_status = LED_SKIN_TEMP_STATUS__NORMAL;
		}
	}
	if (tled_skin_status != chip->led_skin_temp_status) {
		chip->limits.nor_skin_thermal_high_threshold_antishake
				= chip->limits.nor_skin_thermal_high_threshold;
		chip->limits.nor_skin_thermal_pre_high_threshold_antishake
				= chip->limits.nor_skin_thermal_pre_high_threshold;
		chip->limits.nor_skin_thermal_warm_threshold_antishake
				= chip->limits.nor_skin_thermal_warm_threshold;
		chip->limits.nor_skin_thermal_pre_warm_threshold_antishake
				= chip->limits.nor_skin_thermal_pre_warm_threshold;
		if (tled_skin_status > chip->led_skin_temp_status
				&& tled_skin_status == LED_SKIN_TEMP_STATUS__HIGH){
			chip->limits.nor_skin_thermal_high_threshold_antishake
				= chip->limits.nor_skin_thermal_high_threshold - TLED_SKIN_HYSTERISIS_DECIDEGC;
			chip->isSkinTempUp = true;
		} else if (tled_skin_status > chip->led_skin_temp_status
				&& tled_skin_status == LED_SKIN_TEMP_STATUS__LITTLE_HIGH){
			chip->limits.nor_skin_thermal_pre_high_threshold_antishake
				= chip->limits.nor_skin_thermal_pre_high_threshold - TLED_SKIN_HYSTERISIS_DECIDEGC;
			chip->isSkinTempUp = true;
		} else if (tled_skin_status > chip->led_skin_temp_status
				&& tled_skin_status == LED_SKIN_TEMP_STATUS__WARM){
			chip->limits.nor_skin_thermal_warm_threshold_antishake
				= chip->limits.nor_skin_thermal_warm_threshold - TLED_SKIN_HYSTERISIS_DECIDEGC;
			chip->isSkinTempUp = true;
		} else if (tled_skin_status > chip->led_skin_temp_status
				&& tled_skin_status == LED_SKIN_TEMP_STATUS__LITTLE_WARM){
			chip->limits.nor_skin_thermal_pre_warm_threshold_antishake
				= chip->limits.nor_skin_thermal_pre_warm_threshold - TLED_SKIN_HYSTERISIS_DECIDEGC;
			chip->isSkinTempUp = true;
		} else if (tled_skin_status < chip->led_skin_temp_status
				&& tled_skin_status == LED_SKIN_TEMP_STATUS__NORMAL){
			chip->limits.nor_skin_thermal_pre_warm_threshold_antishake
				= chip->limits.nor_skin_thermal_pre_warm_threshold + TLED_SKIN_HYSTERISIS_DECIDEGC;
			chip->isSkinTempUp = false;
		} else if (tled_skin_status < chip->led_skin_temp_status
				&& tled_skin_status == LED_SKIN_TEMP_STATUS__LITTLE_WARM){
			chip->limits.nor_skin_thermal_warm_threshold_antishake
				= chip->limits.nor_skin_thermal_warm_threshold + TLED_SKIN_HYSTERISIS_DECIDEGC;
		...
		chg_debug("tled_skin_status change, [%d %d %d %d %d %d]\n",
				tled_skin_status,
				chip->led_skin_temp_status,
				chip->limits.nor_skin_thermal_high_threshold_antishake,
				chip->limits.nor_skin_thermal_pre_high_threshold_antishake,
				chip->limits.nor_skin_thermal_warm_threshold_antishake,
				chip->limits.nor_skin_thermal_pre_warm_threshold_antishake);
		chip->led_skin_temp_change = true;
		chip->led_skin_temp_status = tled_skin_status;
	}
}

static void oplus_chg_check_led_on_ichging(struct oplus_chg_chip *chip)
{
	if (chip->led_on_change || (chip->led_on && chip->led_skin_temp_change)) {
		chip->led_on_change = false;
		chip->led_skin_temp_change = false;
...
void oplus_chg_variables_reset(struct oplus_chg_chip *chip, bool in)
{
	...
	chip->limits.nor_skin_thermal_pre_warm_threshold_antishake
			= chip->limits.nor_skin_thermal_pre_warm_threshold;
	chip->skin_temp = chip->chg_ops->get_skin_temp();
	pr_err("[OPLUS_CHG][%s]:skin_temp = %d\n", __func__ ,chip->skin_temp);
	chip->led_skin_temp_change = false;
	if (chip->skin_temp > chip->limits.nor_skin_thermal_high_threshold) {
		chip->led_skin_temp_status = LED_SKIN_TEMP_STATUS__HIGH;
	}else if (chip->skin_temp > chip->limits.nor_skin_thermal_pre_high_threshold) {
		chip->led_skin_temp_status = LED_SKIN_TEMP_STATUS__LITTLE_HIGH;
	}else if (chip->skin_temp > chip->limits.nor_skin_thermal_warm_threshold) {
		chip->led_skin_temp_status = LED_SKIN_TEMP_STATUS__WARM;
	}else if (chip->skin_temp > chip->limits.nor_skin_thermal_pre_warm_threshold) {
		chip->led_skin_temp_status = LED_SKIN_TEMP_STATUS__LITTLE_WARM;
	}else{
...
/*	chip->pmic_spmi.usb_online = false;
	...
	chip->boot_mode = 0;//chip->chg_ops->get_boot_mode();
...
{
	charger_xlog_printk(CHG_LOG_FULL, "[BATTERY] Battery full !!\n");
...
static void oplus_chg_protection_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->chg_ctrl_by_lcd) {
		oplus_chg_check_tled_skin_status(chip);
...
static void battery_notify_tbat_check(struct oplus_chg_chip *chip)
{
	...
	if (BATTERY_STATUS__LOW_TEMP == chip->tbatt_status) {
		count_low++;
		charger_xlog_printk(CHG_LOG_CRTI,
				"[BATTERY] bat_temp(%d), BATTERY_STATUS__LOW_TEMP count[%d]\n",
				chip->temperature, count_low);
		if (chip->charger_exist && count_low > 10) {
			count_low =11;
...
static void battery_notify_vbat_check(struct oplus_chg_chip *chip)
{
	...
	if (true == chip->vbatt_over) {
		...
		if ((chip->batt_full) && (chip->charger_exist)) {
			...
			charger_xlog_printk(CHG_LOG_FULL,
...
int oplus_chg_get_prop_batt_health(struct oplus_chg_chip *chip)
{
	...
	} else if (tbatt_status == BATTERY_STATUS__LOW_TEMP || tbatt_status == BATTERY_STATUS__COLD_TEMP) {
...
static int oplus_chg_check_sw_full(struct oplus_chg_chip *chip)
{
	...
	printk("oplus_vbatt_full_vol_sw=%d\n",vbatt_full_vol_sw);
...
static void oplus_chg_check_status_full(struct oplus_chg_chip *chip)
{
	...
	if ((is_batt_full == 1) || (chip->charging_state == CHARGING_STATUS_FULL)
		...
		charger_xlog_printk(CHG_LOG_FULL, "is_batt_full : %d,  chip->charging_state= %d\n", is_batt_full, chip->charging_state);
...
static void oplus_chg_print_log(struct oplus_chg_chip *chip)
{
	...
	if(chip->vbatt_num == 1){
		charger_xlog_printk(CHG_LOG_CRTI,
			...
			"STATUS[ 0x%x / %d / %d / %d / %d / 0x%x / %d ], OTHER[ %d / %d / %d / %d / %d/ %d ]\n",
			...
			chip->tbatt_status, chip->stop_voter, chip->notify_code, chip->bq_enable,
...
static void oplus_chg_pdqc_to_normal(struct oplus_chg_chip *chip)
{
	...
	if (chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_PD) {
		if (chip->batt_volt > chip->limits.vbatt_pdqc_to_5v_thr && (pdqc_9v == true || (chip->chg_ctrl_by_lcd && chip->led_on))) {
			ret = chip->chg_ops->oplus_chg_pd_setup();
			if (ret >= 0) {
				pdqc_9v = false;
				chip->limits.temp_normal_fastchg_current_ma
				...
				oplus_chg_set_input_current_limit(chip);
				if (chip->chg_ctrl_by_lcd && chip->led_on) {
					pdqc_9v = true;      //set pdqc_9v=true, in case we cannot set current when screen off.
				}
			}
		}
...
static void oplus_chg_qc_config(struct oplus_chg_chip *chip)
{
	...
	if (qc_chging == false
		...
		if (ret >= 0) {
			...
			oplus_chg_enable_burst_mode(true);
			if (chip->chg_ctrl_by_lcd && chip->led_on) {
				qc_chging = false;      //set qc_chging=false, in case we cannot set current when screen off.
			}
		}
	}
}
	...
	if (get_eng_version() == HIGH_TEMP_AGING) {
		if (chip->charger_exist) {
			...
			if (chip->ibus < 1200 && chg_count > 100 ) {
				//every 9 minutes to check icl if icl lower than 1.2A set to 1.2A
				charger_xlog_printk(CHG_LOG_CRTI,
					" PQE: led_on = %d, \
					ibus = %d, \
					led_skin_temp_status = %d\n",
...
EXPORT_SYMBOL(oplus_chg_show_warp_logo_ornot);
...
void oplus_smart_charge_by_cool_down(struct oplus_chg_chip *chip, int val)
{
	...
	if (val & SMART_WARP_CHARGER_CURRENT_BIT0) {
		if (oplus_warp_get_warp_multistep_adjust_current_support() == false)
			chip->cool_down = 1;
		else
			chip->cool_down = 1;
	...
	if (val & SMART_COMPATIBLE_WARP_CHARGER_CURRENT_BIT0) {
		chip->limits.input_current_warp_ma_high = OPLUS_CHG_1200_CHARGING_CURRENT;
		chip->limits.input_current_warp_ma_warm = OPLUS_CHG_1200_CHARGING_CURRENT;
		chip->limits.input_current_warp_ma_normal = OPLUS_CHG_1200_CHARGING_CURRENT;
		chip->cool_down_done = true;
	}
	if (val & SMART_COMPATIBLE_WARP_CHARGER_CURRENT_BIT1) {
		chip->limits.input_current_warp_ma_high = OPLUS_CHG_1500_CHARGING_CURRENT;
		chip->limits.input_current_warp_ma_warm = OPLUS_CHG_1500_CHARGING_CURRENT;
		chip->limits.input_current_warp_ma_normal = OPLUS_CHG_1500_CHARGING_CURRENT;
		chip->cool_down_done = true;
	}
	if (val & SMART_COMPATIBLE_WARP_CHARGER_CURRENT_BIT2) {
		chip->limits.input_current_warp_ma_high = OPLUS_CHG_1800_CHARGING_CURRENT;
		chip->limits.input_current_warp_ma_warm = OPLUS_CHG_1800_CHARGING_CURRENT;
		chip->limits.input_current_warp_ma_normal = OPLUS_CHG_1800_CHARGING_CURRENT;
		chip->cool_down_done = true;
		chip->cool_down_force_5v = true;
		chip->chg_ctrl_by_cool_down = true;
	}
