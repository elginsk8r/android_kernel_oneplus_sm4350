	...
	u64 timestamp_us, u64 prev_ts)
{
	...
	if (framerate < 1)
		framerate = 1;
#else /* OPLUS_ARCH_EXTENDS */
	v4l2_fmt = &inst->fmts[OUTPUT_PORT].v4l2_fmt;
	output_size = ALIGN(v4l2_fmt->fmt.pix_mp.height, 16) * ALIGN(v4l2_fmt->fmt.pix_mp.width, 16);
	framerate_limit = (u32) inst->core->resources.max_load * 256 / output_size;
	if(framerate > framerate_limit) {
		s_vpr_h(inst->sid, "%s:force from %d to fps:%d\n",
			__func__, framerate,inst->clk_data.frame_rate >> 16);
