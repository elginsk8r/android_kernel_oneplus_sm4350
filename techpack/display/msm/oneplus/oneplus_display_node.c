...
extern ssize_t oneplus_mipi_dcs_write_buffer(struct mipi_dsi_device *dsi, const void *data, size_t len);
/**
					...
					u16 brightness)
{
	...
	u8 tx_buf[3];
	tx_buf[0] = 0x51;
	memcpy(&tx_buf[1], payload, 2);

	err = oneplus_mipi_dcs_write_buffer(dsi, tx_buf, 3);
		...
		"%04d/%02d/%02d\n%02d:%02d:%02d:%03d.%01d\n%s\nID: %02X %02X %02X\n",
						panel_year, panel_mon, panel_day, panel_hour, 
						    panel_min, panel_sec, panel_msec_int,
								panel_msec_rem, stage_string_info, panel_code_info, panel_stage_info,
										panel_production_info);
    }else if (dsi_panel_name == DSI_PANEL_NT36672C_TIANMA) {
		pr_err("dsi_panel_name == DSI_PANEL_NT36672C_TIANMA\n");
		panel_year = 2020;
		panel_mon = 12;
		panel_day = 2;
		panel_hour = 11;
...
static ssize_t oplus_shutdownflag_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
static ssize_t oplus_shutdownflag_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int flag = 0;
	sscanf(buf, "%du", &flag);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk(KERN_ERR "%s oplus_display_set_cabc = %d\n", __func__, temp_save);
	...
	if (get_oplus_display_power_status() == OPLUS_DISPLAY_POWER_ON) {
	    ...
	    bd = dsi_display->panel->bl_config.raw_bd;
		if(dsi_display->panel->bl_config.type == DSI_BACKLIGHT_WLED)
			if (bd->ops && bd->ops->get_brightness)
				cur_bl_level = bd->ops->get_brightness(bd);

		if(cur_bl_level < 45 && cabc_mode == 1){
			printk(KERN_ERR "cur_bl_level < 45 , cabc_mode do not set on\n");
			cabc_mode = 0;
...
static ssize_t cabc_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	printk(KERN_ERR "oplus_display_get_cabc = %d\n", cabc_mode);
	return sprintf(buf, "%d\n", cabc_mode);
}
